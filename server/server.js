...
const { connectToDB, loadProducts, saveProductStates } = require('./db'); 
...
async function checkPokemonCenter() {
    ...
    try {
        await loadProducts(productStates);
    } catch (dbError) {
        console.error("Initial loading failed:", dbError.message);
    }
    ...
    try {
        await saveProductStates(productStates);
    } catch (dbSaveError) {
        console.error("Failed to save product states to DB:", dbSaveError.message);
        fetchErrors.push(`Database save error: ${dbSaveError.message}`);
    }
